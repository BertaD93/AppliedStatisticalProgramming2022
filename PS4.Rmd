...
endorsements <- Endorsements %>%  rename(candidate_name=endorsee) %>%  as.tibble()
endorsements_count <- endorsements %>%  group_by(candidate_name) %>% summarise(count=n())

spolls <- polls %>%  filter(candidate_name %in% c("Amy Klobuchar", "Bernard Sanders", "Elizabeth Warren", "Joseph R. Biden Jr.", "Michael Bloomberg", " Pete Buttigieg")) %>% select(candidate_name, sample_size, start_date, party, pct) 

spolls <- spolls %>% mutate(new_name = case_when(spolls$candidate_name == "Bernard Sanders" ~ "Bernie Sanders",
                                        spolls$candidate_name == "Joseph R. Biden Jr." ~ "Joe Biden",
                                        TRUE ~ as.character(spolls$candidate_name))) %>% rename(old_name=candidate_name, candidate_name=new_name)
join <- left_join(spolls, endorsements, by="candidate_name")

#No se me ocurre como compararla
# df1 <- Endorsements %>% select(endorsee) %>% distinct() %>% arrange(endorsee) %>%  rename(candidate_name=endorsee) %>% mutate(name=candidate_name)
# df2 <- polls %>% select(candidate_name) %>% distinct %>% arrange(candidate_name) %>% mutate(name=candidate_name)
# join <- full_join(df2, df1, by="candidate_name")
# join_replace <- join %>%  mutate(new_name = case_when(name.x == name.y ~ name.x,
#                                                         is.na(name.y) ~ name.x 
#                                                         )) 

spolls %>%  mutate(new_name = case_when(spolls$candidate_name == "Bernard Sanders" ~ "Bernie Sanders",
                                        spolls$candidate_name == "Joseph R. Biden Jr." ~ "Joe Biden",
                                        TRUE ~ as.character(spolls$candidate_name)))
